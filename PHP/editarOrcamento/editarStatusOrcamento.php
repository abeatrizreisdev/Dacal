...
try {
    ...
    if (isset($_POST['numeroOrcamento']) && isset($_POST['status'])) {
        ...
        if ($resultado) {
            ...
            echo json_encode(['sucesso' => false, 'mensagem' => 'Erro ao atualizar status do orçamento.']);

        }
    } else {

        echo json_encode(['sucesso' => false, 'mensagem' => 'Número do orçamento ou status não fornecido.']);

    }

} catch (Exception $excecao) {

    echo json_encode(['sucesso' => false, 'mensagem' => 'Erro ao atualizar status do orçamento: ' . $excecao->getMessage()]);
